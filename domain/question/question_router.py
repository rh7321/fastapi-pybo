...
def question_list(db: Session = Depends(get_ps_db),
...
def question_detail(question_id: int,page: int = 0, size : int =10, db: Session = Depends(get_ps_db)):
...
def question_create(_question_create: question_schema.QuestionCreate,
    ...
    print(f"question의 현재 유저 아이디는 : {current_user.id}")
...
def notice_list(db: Session = Depends(get_ps_db)):
